...
class primary implements renderable, templatable {
    ...
    protected function get_custom_menu(renderer_base $output): array {
        ...
        if (user_has_role_assignment($USER->id, 5)){
            $CFG->custommenuitems = "Dashboard | /local/student/dashboard.php
            Assignments  | /local/student/assignments.php
            Completed Tasks | /local/student/completedtasks.php
                         Today's Titans | /local/student/todays_titans.php
                                     Course Report | /local/student/course_report.php
            Reports | /local/student/reports.php
            API Reference | /local/student/api.php
            Study Material | /local/student/studymaterial.php
            Feedback | /local/student/feedback.php
    ...
    $CFG->custommenuitems = "Dashboard | /local/teacher/dashboard.php
Daily Performance | /local/teacher/dailyperformance.php

Reports | #
- Performance Reports | /local/teacher/dailyreports.php
- Group Reports | /local/teacher/group-wise-reports.php
- Course Report | /local/teacher/course_report.php

Today's Titans | /local/teacher/todays_titans.php
Course Material | /local/teacher/course_material.php
Assignments | /local/teacher/assignments/index.php
Calendar | /calendar/view.php
";
}
   
        if (user_has_role_assignment($USER->id, 3)&& ($CFG->strictexammode==1)){
            //global $USER;
            //var_dump( user_has_role_assignment($USER->id, 5));
          //  exit(0);
            $CFG->custommenuitems = "Dashboard | /local/teacher/dashboard.php
            Daily Reports | /local/teacher/dailyreports.php
            Daily Performance | /local/teacher/dailyperformance.php
            Course Report | /local/teacher/course_report.php'
            Examinor| /local/teacher/examinor.php
            Course Material | /local/teacher/course_material.php
            ";
...
<style>
    /* Add your custom styles for menu items */
    .custom-menu-item {
    ...
    /* Style the logout item differently */
    .custom-menu-item-logout {
        /* Your styling properties for the logout item */
        color: #ff0000; /* Red color for logout item */
    }
</style>
