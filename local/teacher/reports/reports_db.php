...
//var_dump($submissions);
    ...
    function getGradeByActivity($students,$courseId,$actvalues,$rank)
    {
...
WHERE `instance` ='$current_act[0]' AND `module` ='$modtypeid'");
                        ...
                        $sql = 'SELECT id FROM `mdl_std_activity_attend_tsl`
							...
							AND `aid` =' .$act_id;
  $sql123=$DB->get_records_sql($sql, null);
                     // var_dump( count($sql123));
                        if (count($sql123)) {
                            $att='ABSENT';
                        } else {
                            $att= 'PRESENT';
 ...
 $i=$i+1;
               ...
               // echo $att;
                if (empty($userobj->profile['rollno']))
                    ...
                    $userobj->profile['rollno'] = "<a href=".$CFG->wwwroot.'/report/outline/user.php?id='.$student->id.'&course='.$courseId.'&mode=outline'.">" . $userobj->profile['rollno'] . "</a>";
                ...
                else
{
  $fullname  = substr(  $fullname ,0,20);
                    $fullname = "<a href=".$CFG->wwwroot.'/report/outline/user.php?id='.$student->id.'&course='.$courseId.'&mode=outline'.">"
. $fullname . "</a>";
}
...
//$result['b']="hii";
                        ...
                        $result[$student->id] = array('id'=>$student->id,
                            'rollno' => $userobj->profile['rollno'],
    ...
    function renderOutput($resultObj,$dept,$section,$grade_type,$watchlist)
    {
        ...
        foreach($resultObj as $student)
        {
            ...
            if($student['watchlist']){
                ...
                $reult_html.="<tr><td style='text-align: left !important;'> " . $student['rollno'] . "</td>
 <td style='text-align: left !important;'> " . $student['firstname']. "</td>
 <td>".$student['grade']."</td>
<td> " . $student['rank'] . " </td>
<td> " . $student['dept'] . " </td>
...
<img style='' id='".$student['id']."' data-ref='".$student['watchlist']."' title='".$settitle."' class='watchlist'
  src='".$CFG->wwwroot."/teacher/testcenter/images/".$student['watchlist'].".png'/>
<span style='display:none'>".$student['watchlist']."</span>
   </td></tr>";
...
id='".$student['id']."' data-ref='".$student['watchlist']."' class='watchlist'
src='".$CFG->wwwroot."/teacher/testcenter/images/".$student['watchlist'].".png'/> </td>
</tr>";
                }
            }
            if(empty($dept)&&(!empty($grade_type))&&empty($section)&&$watchlist=='undefined'){
...
src='".$CFG->wwwroot."/teacher/testcenter/images/".$student['watchlist'].".png'/> </td></tr>";
                }
            }
...
<td><img style='width:15px;'
 ...
 src='".$CFG->wwwroot."/teacher/testcenter/images/".$student['watchlist'].".png'/> </td></tr>";
    ...
    /*----------------------------------------------------------------------------render end--------------------------------*/
    function pdfRenderOutput($resultObj,$dept,$section,$grade_type,$watchlist)
    {
        //echo "Grade Type ".$grade_type;
        //echo "section ".$section;
        global $CFG;
...
//$resultArr=$repObj->getGradeByActivity($studnets,"19","33-quiz","");
