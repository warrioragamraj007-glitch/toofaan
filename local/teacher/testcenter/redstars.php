...
$activityid = required_param('id', PARAM_INT);        //activityid
...
echo '<style>.pagecover-onload{
	    display: none; position: absolute; width: 100%; background-color: rgb(255, 255, 255); z-index: 300; opacity: 0.9; height: 800px; top: 60px;margin-left: -10px;
	}
	body{
...
echo '<head><title>Tessellator 5.0 - Testcenter - RedStars</title>
                        <style>
                        .image-caption {
                            color: red;
...
<script>
    ...
    $j(document).on('ready',function() {
        ...
        get_redstars();
        setInterval(get_redstars, 30000);

        function get_redstars()
        {
           // $j(".pagecover-onload").css("display", "block");
            $j.ajax({
                url: baseUrl + "/local/teacher/testcenter/redstar_sub_list.php",
                data: {
                    "cid": cid,
                    "secname": section,
