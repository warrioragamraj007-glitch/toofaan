...
$actid = required_param('actid', PARAM_INT);  // activity id
...
//code to check whether the student is watchlisted or not
        ...
        if($display_flag){
            if($loginstatus) {
                ...
                if(($greenstar==100) && $watchliststatus){
    ...
    if(count($userIdWithSubTime)){
        ...
        echo '<div style="font-size: 22px;font-weight: bold;padding: 100px;color:#FFF">No Red Stars Till Now..</div>';
    }
    echo '</div>';
    ...
    require_once('redstarquizsubmissions.php');
}
