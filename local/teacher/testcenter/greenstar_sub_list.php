...
$actid = required_param('actid', PARAM_INT);  // currentactivityid
$typeid = required_param('typeid', PARAM_INT);    //currentactivity typeid
...
<link rel="stylesheet" type="text/css" href="css/jcarousel.connected-carousels.css">
                ...
                <script type="text/javascript" src="js/jcarousel/jquery.js"></script>
                <script type="text/javascript" src="js/jcarousel/jquery.jcarousel.js"></script>

                <script type="text/javascript" src="js/jcarousel/jcarousel.connected-carousels.js"></script>
<body>';
...
//Get all information
    ...
    foreach ($all_data as $data) {
        $userinfo = $data->userinfo;$greenstar=0;
        ...
        if($data->submission == null){
            ...
            $hrefprev=vpl_mod_href('views/previoussubmissionslist.php','id',$actid,
                'userid',$subinstance->userid,'inpopup',1);
            ...
            if($subinstance->dategraded>0){
                ...
                $gradeTime=$subinstance->dategraded;
            }
            else{
                $result=$submission->getCE();
                $text='';
...
//code to check whether the student is watchlisted or not
        ...
        if($display_flag){
            if($loginstatus) {
                ...
                if(($greenstar==100) && ($watchliststatus==0)){
                    //$statusImag='green-star.png';
                    $starCount++;

                    $userImages[$imgcnt]=$OUTPUT->user_picture($userinfo,array('size'=>420, 'alttext'=>$userinfo->firstname.' ' .$userinfo->lastname, 'link'=>false));
                    $uimg=$OUTPUT->user_picture($userinfo,array('size'=>420, 'alttext'=>$userinfo->firstname.' ' .$userinfo->lastname, 'link'=>false));
                    $userNames[$imgcnt]=$userinfo->firstname.' ' .$userinfo->lastname;
                    $userImagesThmb[$imgcnt++]=$OUTPUT->user_picture($userinfo,array('size'=>50, 'alttext'=>$userinfo->firstname.' ' .$userinfo->lastname,'title'=>'Name:', 'link'=>false));
                    $timg=$OUTPUT->user_picture($userinfo,array('size'=>50, 'alttext'=>$userinfo->firstname.' ' .$userinfo->lastname,'title'=>'Name:', 'link'=>false));
                    $userIdWithSubTime[$gradeTime]=array('uid'=>$userinfo->id,'uname'=>$userinfo->firstname.' ' .$userinfo->lastname,'uimg'=>$uimg,'timg'=>$timg);
...
// echo '<li><div>'.$uiwst['uimg'].'</div><div class="image-caption" style="text-align:center">'.$uiwst['uname'].'</div></li>';
        ...
        echo '<div style="font-size: 22px;font-weight: bold;padding: 100px;color:#FFF">No Green Stars Till Now..</div>';
    }
    echo '</div>';
    ...
    require_once('greenstarquizsubmissions.php');
}

/*
//var_dump($faststudents);
$lb="";
$leaderboard=array();
foreach($faststudents as $fs){
    $leaderboard[]=$fs['uname'];
}
for ($x = 0; $x < 6; $x++){
    $lb=$lb."".$leaderboard[$x].",";
}
$lb=str_replace(",","%2C",$lb);
if(count($faststudents)<=6){
    $str="".strtoupper(str_replace(" ","+",$lb));
    //$firstmsg="http://10.11.38.240/msg?msg=,,,,,,"
    //file_get_contents($firstmsg);
}
$leaderboardmsg="http://10.11.38.240/msg?msg=".$str;

file_get_contents($leaderboardmsg);


echo '<span style="display:none" id="leaderboard-msg">'.$str.'</span>';
echo '<span style="display:none" id="leaderboard-count">'.count($faststudents).'</span>';
*/
?>
</body>
</html>
