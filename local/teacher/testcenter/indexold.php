...
foreach ($mods as $mod) {
    ...
    $arr[$cnt++]=array('secid'=>$mod->section,'modid'=>$mod->id,'modname'=>$mod->name,'modcontent'=>$mod->content,'modtypeid'=>$mod->module,'itemid'=>$gradeitemid,'modstatus'=>$modstatus,'modstop'=>$modstopdate,'modstart'=>$modstartdate,'remoteactivity'=>$remoteactivity);
...
function get_activities($sectionid,$arr)
{
    ...
    foreach($arr as $a){
        ...
        if(($a['secid']==$sectionid)&&($a['modtypeid']!=1)&&($a['modtypeid']!=7)) {
            if ($a['secid'] == $sectionid) {

                $sec_activity_array[$cnt] = array('modid' => $a['modid'], 'modname' => $a['modname'], 'modcontent' => $a['modcontent'], 'modtypeid' => $a['modtypeid'], 'modstatus' => $a['modstatus'], 'itemid' => $a['itemid'], 'modstop' => $a['modstop'], 'modstart' => $a['modstart'], 'remoteactivity' => $a['remoteactivity']);
                $cnt++;
            }
        }
    }
...
echo '<div class="current-activity row">
           <div class="col-md-5 act-label">ACTIVITY NAME : <span class="progress-activity">'.$currentActivityName.'</span></div>
           <div class="col-md-5 act-name">STATUS :<span class="progress-activity-status">'.$currentActivityStatus.'</span></div>
       </div>
            ...
            <ul class="nav nav-tabs course-detail-tabs">
             ...
             <button data-rmodid="' . $activities[$i]['remoteactivity'] . '"  data-mid="' . $activities[$i]['modtypeid'] . '" class="showhide stop hide' . $activities[$i]['modid'] . '" id="hide" value=' . $activities[$i]['modid'];
                     ...
                     echo '>
                             ...
                             <td><i data-mid="' . $activities[$i]['modid'] . '" class="fa fa-users fa-2x students" aria-hidden="true"></i></td>
                            </tr>';
                            ?>
                         </tbody>
               <?php    } ?>
