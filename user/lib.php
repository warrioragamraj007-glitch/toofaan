...
function user_get_user_navigation_info($user, $page, $options = array()) {
    ...
    if ($returnobject->metadata['asotheruser'] = \core\session\manager::is_loggedinas()) {
        ...
        // /**added line to get full name of user beside logout start*/
        // $userFullName = fullname($USER);
        // $logout->title = get_string('logout') . ' (' . $userFullName . ')';
        //  /**added line to get full name of user beside logout end*/
        ...
        if (!$examModeOn) {
            ...
            $logout->url = new moodle_url('/login/logout.php', ['sesskey' => sesskey()]);
            $userFullName = fullname($USER);
            $logout->title = get_string('logout') . ' (' . strtoupper($userFullName) . ')';
            $logout->titleidentifier = 'logout,moodle';
            $returnobject->navitems[] = $logout;
        } else {
            // Exam mode is on, display the name of the user.
            // die("exam mode on");
            global $USER;
            if(user_has_role_assignment($USER->id,5)){
                ...
                $userlink = new stdClass();
                $userlink->itemtype = 'link';
                $userlink->title = strtoupper($userFullName);
                $returnobject->navitems[] = $userlink;
            }
            else{
                global $USER;
                $logout = new stdClass();
