...
function user_login($username, $password) {
       ...
       // Check if user has college profile field set to 1 to restrict password login
       $college_field = $DB->get_record('user_info_data', array(
           'userid' => $user->id,
           'fieldid' => $DB->get_field('user_info_field', 'id', array('shortname' => 'enablefacelogin'))
       ));
      
       if ($college_field && $college_field->data == '1') {
           // Set custom error message for college users and return false
           global $SESSION;
           $SESSION->loginerrormsg = "Password login is restricted for this account. Please use Face Login to continue.";
           return false;
       }
       if (!validate_internal_user_password($user, $password)) {
           return false;
