...
function decrypt($encryptedText){
	...
	foreach ($encryptedChars as $char) {
	    if (isset($decryptionKey[$char])) {
		$decryptedText .= $decryptionKey[$char];
	    } else {
		$decryptedText .= $char;
	    }
	}
...
$errormsg = '';
$errorcode = 0;
// to remove lost password and cookies button
if ($CFG->strictexammode) {
    $CFG->additionalhtmlhead .= '<style>.login-form-forgotpassword { display: none; }</style>';
    $CFG->additionalhtmlhead .= '<style>.btn.btn-secondary { display: block; }</style>';
  } else {
    $CFG->additionalhtmlhead .= '<style>.login-form-forgotpassword { display: block; }</style>';
    $CFG->additionalhtmlhead .= '<style>.btn.btn-secondary { display: none; }</style>';
  }
...
//decrypting from portal form
if($CFG->tesseractmode) {
   if(isset($frm->password)&&isset($frm->reqfrom)){
    //var_dump($frm->username);
    //var_dump($frm->password);
        $password=decrypt($frm->password);
        $username=decrypt($frm->username);
...
echo $OUTPUT->header();
echo '<div class="college-logo" style="text-align: center; margin-bottom: 20px;">
        <img src="'.$CFG->wwwroot.'/pix/kmec-logo.png" alt="College Logo" style="max-width: 200px; height: auto;">
      </div>';
