...
<script>
    ...
    document.getElementById('page-navbar').innerHTML = "<div id='navbar' style='padding: 2% 6% 1% 2%; display: flex; align-items: center;'>  <span style='margin: 0 5px;'></span> <div style='white-space: nowrap; overflow: hidden; width: 400px;font-size: 22px;text-align: end;'>" + activityName + "</div> </div>";
...
<style>
    /* Add a CSS class to hide the element initially */
    .hide-during-refresh {
...
<script>
    ...
    /******** Removed mark as done button start *********/
    document.addEventListener('DOMContentLoaded', function() {
        var regionToHide = document.querySelector('[data-region="activity-information"]');
        if (regionToHide) {
            regionToHide.classList.add('hide-during-refresh');
        }
    });

    // Wait for the element to exist in the DOM
    var checkElementExistence = setInterval(function() {
        var regionToRemove = document.querySelector('[data-region="activity-information"]');
        if (regionToRemove) {
            // Remove the CSS class to show the element
            regionToRemove.classList.remove('hide-during-refresh');
    ...
    /******** Removed mark as done button end *********/
</script>
