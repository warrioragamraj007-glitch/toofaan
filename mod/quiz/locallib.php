...
/*****saving IP address to the database start ********/
function quiz_save_best_grade($quiz, $userid = null, $attempts = array()) {
    global $DB, $OUTPUT, $USER;
    ...
    if (is_null($bestgrade)) {
        ...
        $grade->grade = $bestgrade;
        $grade->timemodified = time();
	/* storing quiz submitted ip start*/
        $quizsubmittedip='NA';
        if($DB->get_field('user', 'lastip', array('id' => $userid))){
            $quizsubmittedip=$DB->get_field('user', 'lastip', array('id' => $userid));
        }
        $grade->subip = $quizsubmittedip;// storing quiz submitted ip
        /* storing quiz submitted ip end*/
        $DB->update_record('quiz_grades', $grade);
...
/*****saving IP address to the database end ***** */
