 ...
 * connection class
 *
 * This class communicates with an tele connect server making REST calls
 * to access the tele Connect API
 */
...
class tconnect_class {
    ...
    public function set_tusername($username = '') {
        $this->_username = $username;
    }
    ...
    /**
     * Posts XML to the tele Connect server and returns the results
...
//            https://example.com/api/xml?action=principal=list
    ...
    public function request_http_header_login($return_header = 0, $username = '', $stop = false) {
        ...
        if (!empty($username)) {
            $hearder = array("$CFG->teleconnect_admin_httpauth: " . $username);
        }
    ...
    public function read_cookie_xml($xml = '') {
            ...
            if (empty($xml)) {
                if (is_siteadmin($USER->id)) {
                    notice(get_string('adminemptyxml', 'teleconnect'),
                           $CFG->wwwroot . '/admin/settings.php?section=modsettingteleconnect');
                } else {
                    notice(get_string('emptyxml', 'teleconnect'),
                           '', $COURSE);
                }
            }
