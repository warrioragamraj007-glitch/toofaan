...
$url = new moodle_url('/mod/teleconnect/participants.php', array('contextid' => $contextid));
...
list($assignableroles, $assigncounts, $nameswithcounts) =
    teleconnect_get_assignable_roles($context, ROLENAME_BOTH, true);
        ...
        if (!empty($userstoassign)) {
            ...
            foreach ($userstoassign as $adduser) {
                ...
                if ($allow) {
                    ...
                    $event = \mod_teleconnect\event\teleconnect_assign_role::create($params);
                    $event->trigger();
                }
            }
            ...
            // $rolename = $assignableroles[$roleid];
            // add_to_log($course->id, 'role', 'assign', 'mod/teleconnect/participant.php?contextid='.$context->id.'&roleid='.$roleid, $rolename, '', $USER->id);
    ...
    if (optional_param('remove', false, PARAM_BOOL) && confirm_sesskey()) {
        ...
        if (!empty($userstounassign)) {
            ...
            foreach ($userstounassign as $removeuser) {
                ...
                $event = \mod_teleconnect\event\teleconnect_unassign_role::create($params);
                $event->trigger();
            }
            ...
            // $rolename = $assignableroles[$roleid];
            // add_to_log($course->id, 'role', 'unassign', 'mod/teleconnect/participant.php?contextid='.$context->id.'&roleid='.$roleid, $rolename, '', $USER->id);
            // Counts have changed, so reload.
            list($assignableroles, $assigncounts, $nameswithcounts) = get_assignable_roles($context, ROLENAME_BOTH, true);
        }
    }
