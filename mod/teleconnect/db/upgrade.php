...
function xmldb_teleconnect_upgrade($oldversion=0) {
...
//===== 1.9.0 upgrade line ======//
    ...
    /// Define field introformat to be added to survey
        $table = new xmldb_table('teleconnect');
    ...
    /// Conditionally launch add field introformat
        ...
        if ($CFG->texteditors !== 'textarea') {
            $rs = $DB->get_recordset('teleconnect', array('introformat'=>FORMAT_MOODLE), '', 'id,intro,introformat');
            foreach ($rs as $s) {
                $s->intro       = text_to_html($s->intro, false, false, true);
                $s->introformat = FORMAT_HTML;
                $DB->update_record('teleconnect', $s);
                upgrade_set_timeout();
            }
            $rs->close();
        }

    /// teleconnect savepoint reached
        upgrade_mod_savepoint(true, 2010120800, 'teleconnect');
    }

    if ($oldversion < 2011041400) {

        // Changing precision of field meeturl on table teleconnect to (60)
        $table = new xmldb_table('teleconnect');
        ...
        // teleconnect savepoint reached
        upgrade_mod_savepoint(true, 2011041400, 'teleconnect');
    }
    ...
    if ($oldversion < 2012012250) {
        ...
        // teleconnect savepoint reached
        upgrade_mod_savepoint(true, 2012012500, 'teleconnect');
