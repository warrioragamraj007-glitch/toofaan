 ...
 */
    ...
    $url = new moodle_url('/mod/teleconnect/conntest.php');
    $PAGE->set_url($url);

    $admins = explode(',', $CFG->siteadmins);

    if (false === array_search($USER->id, $admins)) {
        print_error('error1', 'teleconnect', $CFG->wwwroot);
    }
    ...
    $param = array('name' => 'teleconnect_host');
    $ac->host       = $DB->get_field('config', 'value', $param);

    $param = array('name' => 'teleconnect_port');
    ...
    $param = array('name' => 'teleconnect_admin_httpauth');
    $ac->httpauth   = $DB->get_field('config', 'value', $param);

    $param = array('name' => 'teleconnect_email_login');
    $ac->emaillogin = $DB->get_field('config', 'value', $param);

    $param = array('name' => 'teleconnect_https');
    ...
    foreach ($ac as $propertyname => $propertyvalue) {
        ...
        if ($checkifempty and empty($propertyvalue)) {
            print_error('error2', 'teleconnect', '', $propertyname);
            die();
        }

    }

    $strtitle = get_string('connectiontesttitle', 'teleconnect');
    ...
    $param = new stdClass();
    $param->url = 'http://docs.moodle.org/en/Remote_learner_tele_connect_pro';
    print_string('conntestintro', 'teleconnect', $param);
    ...
    tele_connection_test($ac->host, $ac->port, $ac->login,
                          $ac->pass, $ac->httpauth,
