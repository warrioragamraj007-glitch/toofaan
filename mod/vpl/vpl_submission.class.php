...
class mod_vpl_submission {
     ...
     * @return string
     */
    function print_grade_custom($grade=null){
        $ret = '';
        $inst=$this->instance;
        if($inst->dategraded>0 || $grade != null){
            $vplinstance = $this->vpl->get_instance();
    ...
    function student_print_info($autolink=false){
        ...
        if($autolink){
            ...
            $url=$CFG->wwwroot.'/local/teacher/testcenter/studentsubmissionview.php?id='.$id.'&userid='.$userid.'&submissionid='.$submissionid;
    ...
    public function print_info($autolink = false) {
        ...
        /****** removed download and evaluation links for student in the review pages start  */
        global $USER, $DB;
        ...
        /****** removed download and evaluation links for student in the review pages end  */
        echo '<br>';
        if ( $this->vpl->is_group_activity() ) {
