...
class mod_vpl_submission_CE extends mod_vpl_submission {
    ...
    public static function prepare_execution_info($data) {
        ...
        $info .= vpl_bash_export('VPL_LANG', vpl_get_lang(true));
         # Export user IP
          $info .= vpl_bash_export('TOOFAAN_USER_IP', getremoteaddr() );
		...
		//sending username to jail		
		//global $USER;
		//$info .= vpl_bash_export( 'TESSELLATOR_USERNAME', $USER->username );
            //}
        ...
        if (isset( $options['XGEOMETRY'] )) { // TODO refactor to a better solution.
          global $USER;
            $data->files['vpl_environment.sh'] .= "\n".vpl_bash_export( 'VPL_XGEOMETRY', $options['XGEOMETRY'] );
              $data->files ['vpl_environment.sh'] .= "\n".vpl_bash_export( 'TESSELLATOR_USERNAME', $USER->username );//added for username in jail
        }
    ...
    // vpl_submission.class.php Start  get Studnet Submitted Language  Function Ram Mohan
    function  print_student_language(){
        $subf = $this->get_submitted_fgm();
        $filelist = $subf->getFileList();
        $allowed=array(".php",".java",".cs",".py",".cpp",".c",".cu");
        foreach ($filelist as $filename) {
    ...
    //End  get Studnet Submitted Language  Function Ram Mohan
