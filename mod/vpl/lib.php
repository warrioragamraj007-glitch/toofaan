...
function vpl_user_outline($course, $user, $mod, $instance) {
    ...
    if (! $subinstance) {
        ...
        /****** added to get submission overview by vplreview.php start */
        // $url = $CFG->wwwroot . '/tessellator50/local/student/vplreview.php?id=' . $vpl->get_course_module()->id . '&userid=' . $user->id . '&subid=' . $instance->id;
        /****** added to get submission overview by vplreview.php end */
        // $url = vpl_mod_href( 'forms/submissionview.php', 'id', $vpl->get_course_module()->id, 'userid', $user->id ); //commented original code for removing access to submission
        // $return->info = '<a href="' . $url . '">' . $info . '</a>';
        if(user_has_role_assignment($USER->id,5)){
            $url = $CFG->wwwroot . '/local/student/vplreview.php?id=' . $vpl->get_course_module()->id . '&subid=' . $instance->id;
        }else{
