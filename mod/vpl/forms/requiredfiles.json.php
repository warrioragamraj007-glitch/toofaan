...
require(__DIR__ . '/../../../toofaanlogs.php');
        ...
        case 'save' :
            $postfiles = mod_vpl_edit::filesfromide($actiondata->files);
               foreach ($postfiles as $filename => $content) {
                logMessage("VPL $id uploaded file: $filename");
                // Check if the uploaded file is url.xml to mark the activity as Toofaan.
                if ($filename === 'url.xml') {
                    global $DB;
                    $updated = $DB->execute(
                        "UPDATE {course_modules} SET is_toofaan = 1 WHERE id = :id", 
                        ['id' => $id]
                    );
                    if (!$updated) {
                        logMessage("Failed to mark VPL $id as Toofaan (update returned false)");
                    } else {
                        logMessage("Successfully marked VPL $id as Toofaan");
                    }
                    
                    // $DB->execute("UPDATE {course_modules} SET is_toofaan = 1 WHERE id = :id", ['id' => $id]);
                    // logMessage("VPL $id is marked as Toofaan activity because url.xml file was uploaded.");
                }
                }
